    ...
    let (oprand_type, opcode, reg_value, rm_value, data) = if opcode == "imm_arith" {
        ...
        if rm == 6 {
            ...
            *index += 4;
            ...
            (OperandType::RegMem, opcode, Some(look_up_reg(reg, w)), Some(rm_value), None)
        } else {
            *index += 2;
            (OperandType::RegMem, opcode, Some(look_up_reg(reg, w)), Some(format!("[{}]", MEMORY[rm])), None)
        }
    };
